...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$IDE_BIN_HOME/lib$AGENT_LIB.so" ]; then
    AGENT="-agentlib:$AGENT_LIB=disablej2ee,disablealloc,delay=10000,sessionname=RubyMine70"
  fi
fi

COMMON_JVM_ARGS="-XX:ErrorFile=$HOME/java_error_in_RUBYMINE_%p.log \"-Xbootclasspath/a:$IDE_HOME/lib/boot.jar\" -Didea.paths.selector=RubyMine70 $IDE_PROPERTIES_PROPERTY"
